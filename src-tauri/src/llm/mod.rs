...
struct Args {
    ...
    #[arg(long, default_value = "puffin-phi-v2")]
    model: WhichModel,
...
const PROMPT: &str = r##"
You are an AI model which is writing context of conversation. Conversation gonna append below.
If you don't sure about context of conversesion please don't write anything ?\n Answer:
"##;

pub fn main() -> Result<TextGeneration> {
    use tracing_chrome::ChromeLayerBuilder;
    use tracing_subscriber::prelude::*;

    let args = Args {
        cpu: false,                       // Run on CPU rather than on GPU.
        tracing: false,                   // Enable tracing (generates a trace-timestamp.json file).
        verbose_prompt: false,            // Display the token for the specified prompt.
        prompt: Some(PROMPT.to_string()), // Prompt to generate samples from.
        mmlu_dir: None,                   // Directory containing the MMLU files.
        temperature: None,                // Temperature used to generate samples.
        top_p: None,                      // Nucleus sampling probability cutoff.
        seed: 299792458,                  // Seed to use when generating random samples.
        sample_len: 5000,                 // Length of the sample to generate (in tokens).
        model_id: None,                   // Model ID to use.
        model: WhichModel::PuffinPhiV2,   // Model to use.
        revision: None,                   // Revision of the model to use.
        weight_file: None,                // Weight file to use.
        tokenizer: None,                  // Tokenizer file to use.
        quantized: true,                  // Use quantized model.
        repeat_penalty: 1.1,              // Penalty to be applied for repeating tokens.
        repeat_last_n: 64,                // Context size to consider for the repeat penalty.
        dtype: None,                      // Dtype to be used for running the model.
    };
    ...
    Result::Err(anyhow::Error::msg("message"))
}
